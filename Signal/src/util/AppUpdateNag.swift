...
class AppUpdateNag: NSObject {
    ...
    public func showAppUpgradeNagIfNecessary() {
        ...
        firstly {
            ...
            // If someone is building Link Messenger with their own bundleID, it's less important that this succeeds.
            if bundleIdentifier.hasPrefix("org.whispersystems") {
                owsFailDebug("Failed to find Link Messenger app store record")
            } else {
