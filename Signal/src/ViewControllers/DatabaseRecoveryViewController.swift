...
class DatabaseRecoveryViewController: OWSViewController {
    ...
    private func renderDeviceSpaceWarning() {
        ...
        headlineLabel.text = NSLocalizedString(
            ...
            comment: "On the database recovery screen, if the user's device storage is nearly full, Link Messenger will not be able to recover the database. A warning screen, which can be bypassed if the user wishes, will be shown. This is the title of that screen."
        )

        descriptionLabel.text = {
            let labelFormat = NSLocalizedString(
                "DATABASE_RECOVERY_MORE_STORAGE_SPACE_NEEDED_DESCRIPTION",
                comment: "On the database recovery screen, if the user's device storage is nearly full, Link Messenger will not be able to recover the database. A warning screen, which can be bypassed if the user wishes, will be shown. This is the line of text on that screen. Embeds an amount like \"2GB\"."
            )
        ...
        let continueButton = button(
            title: NSLocalizedString(
                ...
                comment: "On the database recovery screen, if the user's device storage is nearly full, Link Messenger will not be able to recover the database. A warning screen, which can be bypassed if the user wishes, will be shown. This is the text on the button to bypass the warning."
            ),
